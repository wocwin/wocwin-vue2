...
export default {
  ...
  methods: {
    ...
    async systemIdChange(systemId) {
      ...
      const res = await this.$api.menuTreeselect({ systemId })
      if (res.success) {
    ...
    async roleMenuTreeselect(roleId, systemId) {
      ...
      const res = await this.$api.roleMenuTreeselect(params)
      if (res.success) {
    ...
    // 选择部门重新赋值
    deptIdsChange(item) {
      console.log('选择部门重新赋值', item)
      console.log('选择部门重新赋值---222', this.formOpts.formData.deptIds)
    ...
    submit() {
      ...
      this.formOpts.ref.validate(async (valid) => {
        ...
        if (this.title == '新增') {
          console.log('新增--最终传参', formData)
          this.$message.success(`新增成功`)
          this.rest()
          this.$emit('submit')
        }
        if (this.title == '编辑') {
          console.log('编辑--最终传参', formData)
...
.add_edit {
	::v-deep .t-form .slot_label .el-form-item__content label {
		min-width: 0;
	}
}
</style>
